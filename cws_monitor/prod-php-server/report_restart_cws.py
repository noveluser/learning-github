...
def check_port_activity(port):
    ...
    while num < 3:
        ...
        if Pro_status == []:
          log(log_file,port+"端口未能检测到","cws节点未能启动")
          cws_restart(path1)
          num = num +1
        else :
          num = 100
          log(log_file,port+"端口检测成功","cws节点已启动")
    if num == 4 :
       sendmail('ERROR','无法自行启动，请尽快人工干预！！！',receivers,0)
    log(log_file,"  重启次数num=   ",str(num))
    return             

def locate_cws_path(port):
    if port == '9002':
        cws_path="/data/cyy928/stag/cws"   #测试stag环境
    if port == '9003':
	...
	with open(t,'r') as f:
            ...
            for x in lines:
 		    ...
 		    restart_status=True     #play_status.log文件找不到active count,状态文件未能输出，那么也重启
                                    #这条else语句有问题，当for循环时，总有搜素不到的时候，那么意味着restart_status一直为真，后期再优化
...
def check_restart(log_file,check_file,port):    #检查是否已经重启过的函数，如果已经重启过，则跳过重启函数
     ...
     if int(flag) == 0:
         ...
         check_port_activity(port)   #检测服务是否启动，如果未能启动，再次重启，重复三次,三次不行，等待人工干预
	 #time.sleep(10)
...
#ports=['9000']
ports=['9003']
path1="/data/cyy928/crontab/"    #监控程序所在目录,crond为QA环境，crontab为PROD环境
